...
/*   Updated: 2025/02/09 14:26:42 by kalipso          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	h_txt_plane(t_calcul_px *calcul, t_plane *plane);
void	h_nmap_plane(t_calcul_px *calcul, t_plane* plane);
...
int	h_dist_plane(t_calcul_px *calcul, t_plane *plane, t_plane_calc* c, int simple)
{
	...
	if (plane->param.texture)
		h_txt_plane(calcul, plane);
	if (plane->param.normal_map)
		h_nmap_plane(calcul, plane);
...
void	h_txt_plane(t_calcul_px *calcul, t_plane *plane)
{
	...
	t_vect	o_to_inter = vect_ab(&calcul->inter, &plane->O.c0);

	double	u = ft_dot_product(&o_to_inter, &plane->O.right) * (plane->param.gamma);
	double	v = ft_dot_product(&o_to_inter, &plane->O.up) * (plane->param.gamma);

	int text_x = ((int)floor(u) % txt->sz_x + txt->sz_x) % txt->sz_x;
	int text_y = ((int)floor(v) % txt->sz_y + txt->sz_y) % txt->sz_y;
...
void	h_nmap_plane(t_calcul_px *calcul, t_plane* plane)
{
	...
	double	u = ft_dot_product(&o_to_inter, &plane->O.up);
	double	v = ft_dot_product(&o_to_inter, &plane->O.right);

	int text_x = ((int)floor(u) % nmap->sz_x + nmap->sz_x) % nmap->sz_x;
	int text_y = ((int)floor(v) % nmap->sz_y + nmap->sz_y) % nmap->sz_y;
